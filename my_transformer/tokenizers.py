...
# %% auto 0
__all__ = ['txt', 'toks', 'tk', 'text', 'txt_to_enc', 'ByteLevelPreTokenizer', 'ByteLevelDecoder', 'Encoding', 'BPETokenizer']
...
class BPETokenizer:
    ...
    def encode(self, txt: str) -> Encoding:
        ...
        # TODO we need to assign each token a position start/end in the original text. We decompose
        encoded = [self.vtoi.get(c, 0) for w, p in word_splits for c in w]
        offsets = []
        ...
        enc = Encoding(ids=encoded, tokens=word_splits, offsets=offsets)
        return enc
...
tk.train(text)
txt_to_enc = "Hello, I love to test this new thing"
tk.encode(txt_to_enc)
