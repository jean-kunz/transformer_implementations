...
# %% auto 0
__all__ = ['tk', 'sents', 'text', 'text_enc', 'ByteLevelPreTokenizer', 'BPETokenizer']

# %% ../notebooks/tokenizers.ipynb 3
from collections import defaultdict
import re

# %% ../notebooks/tokenizers.ipynb 6
class ByteLevelPreTokenizer:
    def __init__(self, add_prefix_space=False):
        self.add_prefix_space = add_prefix_space

    def pre_tokenize_str(self, sentence: str) -> list[str]:
        if self.add_prefix_space:
            sentence = " " + sentence
        raw_tokens = [t for t in re.split(r"(\s|,|\.|;|:|!|\?|\t)", sentence) if t]
        prev_t = None
        tokens: list[str] = []
        for t in raw_tokens:
            if t == " ":
...
for s in sents:
    gpt_pre_tokenize = [
        t for t, p in tokenizer.backend_tokenizer.pre_tokenizer.pre_tokenize_str(s)
    ]
    assert (
        tk.pre_tokenizer.pre_tokenize_str(s) == gpt_pre_tokenize
    ), f"{tk.pre_tokenizer.pre_tokenize_str(s)} != {gpt_pre_tokenize} (gpt byte level pretokenizer)"


text = "A Hello world, this is a test. A new world is coming, Hell yes ."
tk.train(text, vocab_size=100)
